...
def downloadFromTikTok(hashTag):
    cookie = {
    ...
    tiktoks = api.getVideosByHashTag(hashTag, count=config.tiktokCount)

    #print(tiktoks)

    tikTokList = tiktoks["itemList"]

    for i in range(len(tikTokList)):
        print(tikTokList[i])
        #data = api.get_Video_By_TikTok(tiktoks[i])# bytes of the video
        data = api.downloadVideoById(tikTokList[i]["id"], "/Users/tylerkirkpatrick/projects/combineClipsAndUpload/clips/output_{}.mp4".format(i))
        #with open("clips/{}.mp4".format(str(i)), 'wb') as output:
            #output.write(data)
